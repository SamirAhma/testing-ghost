...
const parseOptions = require(`gatsby-rehype-prismjs/parse-options`);

const loadLanguageExtension = require(`gatsby-rehype-prismjs/load-prism-language-extension`);

const highlightCode = require(`gatsby-rehype-prismjs/highlight-code`);

const addLineNumbers = require(`gatsby-rehype-prismjs/add-line-numbers`);

const commandLine = require(`gatsby-rehype-prismjs/command-line`);
...
module.exports = ({ htmlAst }, pluginOptions) => {
    ...
    const inlineHighlight = (node) => {
        ...
        const codeHtml = `<code class="${className}">${highlightCode(
            languageName,
            value,
            escapeEntities
        )}</code>`;
